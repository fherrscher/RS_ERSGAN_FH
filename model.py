...
class Generator(nn.Module):
    def __init__(self, in_channels=4, num_channels=64, num_blocks=23):
        super().__init__()
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=4, features=[64, 64, 128, 128, 256, 256, 512, 512]):
        super().__init__()
        blocks = []
...
def test():
    ...
    disc = Discriminator()
    low_res = 24
    x = torch.randn((5, 4, low_res, low_res))
    gen_out = gen(x)
